...
| 리포 서가 | 서가 화면 + 마스트헤드 스위처 + `repo.overview` 한 방 쿼리 | `be60172` |
| 클론 코스 | `0003 clone_run·clone_step`, 커밋 순/위상 폴백 순서 산출, 판 생성, 채점 | `4d89264` |
| 문구 전수(굳은 영역) | 443키를 `t()` 로 — session 278 · cards 93 · grading 30 · ui 42 | `3e7bfea` |
...
- **병합이 열려 있을 때 커밋하면 그 병합이 닫힌다** — `be60172` 는 제목이 lint 인데 내용은 서가 병합이다. 내 실수이고 아직 푸시 전이다.

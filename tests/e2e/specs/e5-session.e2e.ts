 ...
 * T2 배치 → 요약 (06 §1.5).
 *
 * **절반만 만들었다.** 창에서 실제로 확인되는 것은 학습 화면이 열리고 · 고르기와 제출이 판정을
 ...
 *   2) **T1·T2 문제가 시드에 없다.** 시드는 `fixtures/ipc/tiny` 덤프 한 파일에서 파생한다 —
 *      T2 는 파일 하나로 지도를 못 그리고(D103 이 `two-commits` 에서 적은 것과 같은 이유),
 *      T1 은 04 §3.1 순위 ②(「모르는 문법 ≤ 3개」)가 갓 만든 숙련도에서 후보를 0 으로
 *      만든다. `perfRun.ts` 가 그 「며칠 뒤」를 억지로 만드는 코드를 들고 있으니(숙련도 2단계
...
describe('E5 세션 전 흐름', () => {
  ...
  it('학습 화면이 열린다 (05 §2.3 — 라우트가 아니라 오버레이)', async () => {
    const proof = await shown('.proof');
    assert.equal(await proof.getAttribute('aria-modal'), 'true', '학습 화면이 모달이 아니다');
    await shown('.ps');
  });
  ...
  it('「모르겠어요」가 4단 사다리를 편다 (정본 §3-1)', async () => {
    ...
    assert.ok(
      (await ladder.getText()).includes('모르겠어요 = 다시 풀기'),
      '사다리 제목이 다르다',
    );
  });
  ...
  pending(
    '사다리 4단 → 아래층 점프 → 자동 복귀',
    '아래층 문제가 시드에 없다(개념 한 파일치) — 점프가 갈 곳이 없다',
  );
  pending(
    'T1 필사 채점 → T2 배치 → 요약',
    'T1·T2 문제가 시드에 없다 — 파일 머리 (2) 참고',
  );
});

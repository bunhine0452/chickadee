/**
 * E6 Esc 복구 — 3번째 문제에서 Esc → 홈 → 재진입 시 3번째 문제부터, 입력 중 Esc 는 입력만
 * 빠져나감 (06 §1.5 · 02 §5.6).
 *
 * 「몇 번째 문제인가」는 홈이 스스로 말한다 — 「이어 풀기 · N번째 문제부터」(`TodayPanel`). 그래서
 * 이 시나리오는 스토어를 들여다보지 않고 **버튼 문구**로 판정한다.
 *
 * 큐 길이는 데이터가 정한다. 시드 큐가 3개보다 짧으면 목표를 마지막 문제로 낮춘다 — 문제
 * 번호 3 은 시나리오의 뜻(「중간에서 나갔다 그 자리로 돌아온다」)이지 상수가 아니다.
 * 「입력 중 Esc 는 입력만」은 T1 필사 문제(Monaco)가 있어야 하는데 시드에 없다(E5 머리 참고).
 */
import { strict as assert } from 'node:assert';

import { before, describe, it, pending, shown, waitForBoot, wd } from '../helpers/driver.js';

/** 한 문제를 답하고 다음으로 넘긴다. 정답 여부는 보지 않는다(E5 머리 (1)). */
...
describe('E6 Esc 복구', () => {
  ...
  it('3번째 문제(큐가 짧으면 마지막)에서 Esc 로 나오면 홈이 그 자리에 있다', async () => {
  ...
  pending(
    ...
    'T1 필사 문제(Monaco)가 시드에 없다 — E5 머리 (2) 와 같은 이유',
  );
});

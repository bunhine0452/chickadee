 ...
 *
 * 이 파일이 지켜야 하는 것은 하나다: **끝이 있다.** 그 끝은 둘이 정한다 — 담기는 개념이
 * `essential ∧ 선행 깊이 ≤ {@link ZERO_CHAPTER_MAX_DEPTH}` 라는 **유한 집합**인 것과,
 * {@link isDone} 의 조건 셋. 판 수의 상수 상한은 없다(D184 — 24 였던 것을 폐지). 열 언어의
 * 0부가 들어오면서 후보가 28~34 로 언어마다 달라져 어떤 상수도 「무엇을 자를까」를 임의로
 * 만들었고, 자르지 않는 것이 그 임의성을 없애는 길이었다. 「일반 튜토리얼로의 변질」을
 * 막는 것은 상수가 아니라 D137 — 리포에 예고할 자리가 없는 개념은 들어오지 않는다.
 *
 * 담기는 것의 우선순위 (D136):
 ...
 *
 * 1 → 2 (D147). 상한 24 가 있던 동안은 「후보가 상한 언저리에 오게」 고르는 값이었다.
 * 상한이 없어진 뒤(D184)로는 이것이 0장에 드는 개념의 **유일한 선별 규칙**이다 — 뿌리에서
 * 두 단까지가 「바닥」이다. 그 언어를 이미 아는 사람은 {@link isDone} 의 둘째 조건(뿌리
 * 4장 중 3장)으로 네댓 장에서 빠져나가 전부를 보지 않는다 — 그 조건이 초보에게만 꺼진다.
 */
...
/**
 * 0장에 담을 판을 순서대로. 후보 전부다 — 자르지 않는다 (D184).
 *

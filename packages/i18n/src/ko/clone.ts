 ...
 *
 * 자리 이름이 `clone.` 이 아니라 **`course.`** 인 이유: `clone.` 은 이미 T1 문제의 문구가
 * 60여 키로 쓰고 있고(`ko/session.ts`), 두 파일이 같은 이름을 내면 `ko.ts` 의 전개에서
 * 나중 것이 앞 것을 조용히 덮는다. 코스 화면은 그 문제를 **재사용**하므로 그 문구는
...
export const clone = {
  ...
  'course.doneBody': '{{n}}개 조각을 필사했습니다. 숙련도는 이미 올랐습니다.',
  'course.doneBack': '홈으로',
  'course.noConceptNote':
    '이 조각에는 사전이 아는 문법이 없어 숙련도는 오르지 않습니다. 점수만 남습니다.',

  // ───────── 진입 ─────────
  'course.openOn': '「{{name}}」 코스 열기',
  'course.inSession': '학습 중에는 코스를 열 수 없습니다. 먼저 세션에서 나와 주세요.',
} as const;

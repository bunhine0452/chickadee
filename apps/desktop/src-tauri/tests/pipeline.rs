...
fn a_real_repository_ingests_within_the_budget() {
    ...
    // 시간은 **찍기만 한다**. 03 §7 의 15s 예산은 기준 기계의 **릴리스** 빌드 수치이고
    // `cargo test` 는 디버그 빌드다 — 디버그 tree-sitter 는 몇 배 느리고, CI 러너의
    // 코어 수도 개발기와 다르다. 여기서 벽시계를 단정하면 코드가 아니라 러너를 재게 된다.
    // 예산을 지키는 자리는 `scripts/bench.sh`(릴리스 · `bench/baseline.json` · +30 % 게이트)다.
}

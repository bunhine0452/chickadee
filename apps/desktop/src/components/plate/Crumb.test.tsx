...
describe('Crumb', () => {
  it('목업 클래스를 그대로 붙인다', () => {
    const { container } = render(<Crumb depth="아래층" parent="옵셔널 체이닝 ?." onBack={() => undefined} />);
  ...
  it('아래층이면 위 판 이름을 앞에 세운다', () => {
    const { container } = render(<Crumb depth="아래층" parent="옵셔널 체이닝 ?." />);
    expect(container.querySelector('.crumb b')?.textContent).toBe('옵셔널 체이닝 ?.');
  });

  it('다시 찍기는 위 판이 없고 기본 문구를 쓴다', () => {
    const { container } = render(<Crumb depth="다시 찍기" />);
  ...
  it('「위로」 버튼은 onBack 이 있을 때만 나오고 B 를 함께 적는다', () => {
    render(<Crumb depth="다시 찍기" />);
    expect(screen.queryByRole('button')).toBeNull();
    cleanup();

    render(<Crumb depth="아래층" parent="위 판" onBack={() => undefined} />);
  ...
  it('「위로」를 누르면 onBack 을 부른다', async () => {
    ...
    render(<Crumb depth="아래층" parent="위 판" onBack={onBack} />);
  ...
  it('설명은 갈아 끼울 수 있고 서식은 정화한다', () => {
    const { container } = render(<Crumb depth="아래층" note={'<b>지금</b> · 1문제<script>x</script>'} />);

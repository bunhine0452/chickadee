...
| D68 | Rust 줄 예산 실측 2028 — **사용자 결정 대기** | 게이트 상한을 **잠정 2100** 으로 올리고 크레이트별 내역을 매 실행 출력한다. 01 §1.1 의 1500 과 정본 §5 의 「500~1500줄」은 **아직 고치지 않았다** — 정본 변경은 §7-2 에 따라 사용자 확인이 먼저다. 남은 마일스톤이 더할 양: `parse_snippet` M3 ~25 · `git_diff_text` M4 ~67 · `dict_*` M5 ~65 · `repo_glob_read` M6 ~30 → MVP 예상 ~2,220 | 실측: `git 383` + `parse 358` + `store 343` + `app 944` = 2,028. 01 §4 는 앱을 300줄로 잡았는데 M0 골격만으로 이미 215줄이었고, 잡 러너 하나가 순회·필터·해시 증분·파서 풀·500행 tx·진행 이벤트·취소를 들고 있다. D64~D67 로 명령 10개를 TS·다음 마일스톤으로 덜어낸 뒤에도 463줄이다. 1500 에 맞추려면 인제스트 자체를 빼야 하고 그러면 M1 이 없다. **줄 수는 대리 지표였고, 실제 방벽인 도메인 어휘 금지·SQL 리터럴 금지·git 바이너리 금지·`1 크레이트 = 1 래핑`·`forbid(unsafe_code)`·`clippy::pedantic` 은 그대로 초록이다** — Rust 는 여전히 개념·판·겹·세션을 모른다 | `scripts/check-rust-budget.sh` · 01 §1.1·§4 · 정본 §5 |
...
**미확인 1건 (M1 이 올림 · D68).** 정본 §5 「얇은 Rust … 500~1500줄」 → 실측이 2,028 이고 MVP 까지 ~2,220 이 예상된다. 사용자가 승인하면 정본의 수치를 고치고 01 §1.1·§4 를 따라 맞춘다. 승인 전까지 게이트는 잠정 2100 이며 `docs/**` 의 1500 표기는 그대로 둔다.

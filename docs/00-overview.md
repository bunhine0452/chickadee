...
D48~D62 는 M0(골격), D63~D71 은 M1(인제스트), D72~D78 은 M2(T0 세션)에서 나왔다.
...
| D80 | WKWebView 프레임 실측 — **예산 초과** | 05 §10 의 홈 프레임 p95 예산 **12 ms** 에 대해 실측은 **18 ms**(warm, 178프레임 · 노드 384개)다. 나머지는 통과: `home:paint` 106 ms(예산 400) · `session:mount` 4 ms(50) · `t0:grade` < 1 ms(30) · `theme:switch` 48 ms(100) · `lifer:open` 13 ms(50). `t1:monaco`(250)는 M3 가 계측한다. **첫 실행은 blame 2차 패스가 배경에서 도는 동안 33~38 ms 까지 오른다** — 위 18 ms 는 인제스트가 끝난 뒤(warm)다. **정본 §6 의 「Chromium 49노드 p95 8.5ms 확인, WebKit 미측정」은 이제 사실이 아니다** — 고치려면 §7-2 대로 사용자 확인이 필요하므로 여기까지만 적는다 | 재는 자를 앱 안에 두고(`VITE_PERF=1` 하네스) 릴리스 빌드를 격리된 데이터 디렉터리로 띄워 쟀다. 노드가 49개가 아니라 **384개**(대지 18장)라 같은 조건이 아니고, WebKit 이 Chromium 보다 느린 것까지 겹친다. 05 §10 은 이 경우의 강등 사다리(윈도잉 → 판번호 어긋남 끄기 → 결 op 0)를 이미 갖고 있으므로 규칙을 고칠 것이 아니라 그 사다리를 밟을 일이다 — 어느 단인지는 D81 이 재서 정했다. **남은 것은 정본 §6 한 문장뿐이고 §7-2 대로 사용자 확인이 필요하다.** 바꿀 문구: 「Chromium 49노드 p95 8.5ms 확인, WebKit 미측정」 → **「Chromium 49노드 p95 8.5ms · WebKit(WKWebView) 384노드 p95 18ms 확인 — 예산 12ms 초과, 윈도잉으로 내린다(D80·D81)」** | 05 §10 · 정본 §6 · `apps/desktop/src/devtools/` |

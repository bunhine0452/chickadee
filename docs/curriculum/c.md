...
800줄 상한을 넘어 [`c-learning.md`](./c-learning.md) 로 분리했다 — 기계 **둘**(자리·약속)과 그것이
덮는 오개념 수 · 교재 다섯의 순서 실측 · UB 판정이 T1 을 통과하는 이유 · 오답 분류 셋 신청 ·
「C 를 코스에서 빼고 `cs/` 의 실습 표기로」 권고 · 바꿀 것 diff.

...
**본문 다섯을 전부 반영했다** (2026-09-05 · D187 ⑰). 나머지 셋은 `docs/curriculum/**` 밖이거나
등록부다.

| 어디 | 무엇 | 왜 | 반영 · 어디 |
|---|---|---|---|
| §0.3 옵셔널 문단 | 「0부는 문만 연다」에 **정본 둘의 배치**(TSPL 1장 열두 번째 절 · 100 Days 14일째)를 근거로 단다 | §11.2 — 우리 논증이었던 것에 외부 근거가 붙었다 | **반영** · `swift.md` §0.3 「0부가 옵셔널을 안 갖는 이유」 뒤 문단 |
| §0.1 0-5 | 「우선순위가 표준 라이브러리 선언에 있다」에 **애플 책은 마지막 장에 둔다**는 사실과 **우리가 앞에 두는 이유**를 한 줄 | §11.2 갈림 ① — 정본과 반대인 자리는 이유를 적어 둔다 | **반영** · `swift.md` §0.1 0-5 행의 「한 줄」 칸 |
| §3 ⑤ `swift/optional-type` | **「옵셔널은 겹친다」**를 항목에 더한다 — `?.` 는 평탄해지고 첨자는 안 그렇다 | 실측. `y["k"] ?? 7` 이 `nil` 인 것이 이 언어에서 가장 좋은 `value` 판이다 | **반영** · `swift.md` §3 5행 끝 |
| §9 오개념 | ①②③ 을 **「겹치는 옵셔널」** 한 줄로 보강한다. 열둘을 늘리지 않는다 | 근거가 실측이고 §9 는 이미 옵셔널에 셋을 썼다 | **반영** · `swift.md` §9 에 「1·2·3 보강」 행 + 「열둘을 늘리지 않았다」 한 줄 |
| §0.2 형식 표 | 0-1·0-3·0-6 옆에 **`event` 값(`compile error`)** 을 부기 | 실측 넷이 값이 아니라 컴파일 정지다 | **반영** · `swift.md` §0.2 표 세 행 + 뒤 문단 |
| `packages/cards/src/fundamentals.ts` | `EVENTS` 에 `compile error` · `FundItem` 에 **`variants`** 필드 | Go 편이 같은 것을 요구한다. 정적 타입 언어 공통이라 I1 의 자리다 | **범위 밖** · `packages/cards`. `swift.md` §0.2 에 「신청했다」로 적어 뒀다 |
| `design/system/diagrams.md` | **신청 없음** | 그림 여섯이 전부 서고 옵셔널은 걸음 사다리가 받는다(§11.3.2) | **반영(변경 0)** · README §12 신청 표에 Swift 행이 없다 |
| **등록부 초안** (번호 미정 · 마지막이 D184) | **오답 진단의 재료를 둘로 한다 — `siblings`(다른 언어의 답)와 `variants`(같은 코드의 한 글자 다른 판).** Swift 일곱 중 둘만 `siblings` 로 잡히고 셋이 `variants` 를 요구하며, SQL 이 독립적으로 같은 결론에 닿았다(`NOT IN` ↔ `NOT EXISTS`). 진단문은 여전히 사람이 안 적는다 | 실측 — `Int8(127)+1` 의 오답 `-128` 은 어느 언어의 답도 아니라 `&+` 의 답이다 | **범위 밖(등록부)** · curriculum `README.md` §13 이 `variants` 를 「확장 여섯」에 이미 적었다 |

...
**합성 예제도 같은 사슬을 탄다 (D137).** `makeSyntheticCard` 는 사전 예제(`examples[].code` + `expect.picks`)를 사용처 모양으로 싸서 `prefer(ly)` 를 그대로 돌린다 — 유형을 고정하지 않는다. 실측이 그 이유다: 0장이 담는 뿌리 개념 넷 중 `ts/string-literal`·`ts/number-literal`·`ts/undefined-null` 은 사전에 `point:` 문항이 **0개**고, `ts/const-declaration` 은 문항이 있어도 예제 `const total = 42` 에서 짚을 후보가 셋뿐이라 §1.1 이 요구하는 넷을 못 채운다. ts `essential` 22개 기준 `meaning:` 22 · `point:` 19 · `blank:` 2 다. 사전이 채워지면 같은 개념이 저절로 지목형으로 올라선다 — 생성기를 고칠 필요가 없다.
...
| 2 아래층 진단 | `prereq[]` 각각 `{name, ly, status, cardId?}` | status: `known`(ly ≥ 2) · `gap`(ly ≤ 1, 카드 생성 가능 → 즉석 `prereqOnly` 카드, `mins 0.7`) · **`preview`**(리포에 사용처는 있으나 생성 불가 → **사전 예제 합성 판 + 그 사용처 예고**) · `none`(리포에 사용처가 없음 → 「판 없음」, **아무것도 만들지 않는다**). **D137 이 이 두 갈래의 방향을 바로잡았다** — 전에는 리포에 없을 때 합성을 내라고 적혀 있었는데, 그러면 「곧 네 코드 어디에서 이걸 보게 된다」의 **어디**가 없어 방안 E-4 의 조건을 지킬 수 없다. 합성 판을 **맞히면** 예고한 사용처의 카드가 그날 큐에 `role='gap'` 으로 들어간다 — 예고를 문장이 아니라 큐로 지킨다. 생성기는 `packages/cards/src/t0-synthetic.ts` 이고 `previewSiteId` 가 필수 인자다. 점프·복귀 후 문단 = 현재 개념의 `payoff` + 선행 개념의 `bridge`(없으면 일반 문장) |
...
- `patternKey = hash(grammar, **편집 보조**, 정렬된 reason 코드, 형태 서명)` · 형태 서명 = 토큰 종류 열(식별자→`I`, 리터럴→`L`, 연산자는 그대로) 원본↔답안. 「규칙 제안」은 상태가 아니라 **파생** — 같은 `patternKey` 의 `open` 행이 **3건 이상**이면 설정 화면에 뜬다: 예시 3건(로컬 코드) + 정규화 **카탈로그**(기본 꺼짐, 언어별 토글: `TRAILING_COMMA_ARGS` · `ARROW_BODY_BRACES` · `OPTIONAL_PARENS_RETURN` · `SELF_CLOSING_JSX` · `QUOTE_IN_JSX_ATTR` …) 중 매칭되는 것을 켜자고 제안. 매칭 없으면 「이슈로 보내기」.
- **절대 동등이 될 수 없는 목록**(카탈로그 밖): 연산자 변경(`==`↔`===`, `&&`↔`||`), 인자 순서, 호출 유무, 템플릿↔연결, `await`·`?.` 유무, 리터럴 값, 이름 맞바꿈.
- **편집 보조가 키에 들어간다 (D143).** 설정으로 보조를 끄고 켤 수 있게 되면서 같은 85%가 서로 다른 조건에서 나온 값이 됐다 — 조건을 키에 안 실으면 「같은 불만 3건」이 사실은 서로 다른 3건이 되고, 그 3건으로 켜자고 제안한 규칙은 아무 조건에서도 맞지 않는다. `draftAppeal(row, original, user, grammar, assist)` 의 다섯째 인자가 그 값이고 생략하면 `'stage'`(기본)로 친다. **이 한 줄이 기존 키를 전부 바꾸므로 `T1_ENGINE_VERSION` 을 `'1'` → `'2'` 로 올렸다** — 열려 있던 누적 묶음은 흩어지고 새 판본에서 다시 모인다.
...
### 7.5 리포 지도 — 노드가 파일이 아니라 대지·폴더 (D142)

`buildGraph({ scope: 'repo' })`. §7.4 의 지도는 대지 + 1-hop 이라 「이 프로젝트는 이런
구조구나」에 답하지 않는다 — 그 물음에 답하는 그림은 리포 전체인데, 파일 2,000장을 24 노드에
넣으려면 뺄 것을 고르는 수밖에 없다. **접으면 아무것도 빼지 않고 전체 모양이 남는다.**

| | 규칙 | 왜 |
|---|---|---|
| 노드 | 대지 뿌리 폴더. 어느 대지에도 안 드는 파일은 제 디렉터리. 리포 뿌리 바로 밑 파일은 파일 그대로 | 대지 판정은 `assignUnits`(03 §6.5 · D29) **그대로 쓴다** — 홈이 인쇄 시트를 세는 규칙과 지도가 폴더를 세는 규칙이 갈라지면 같은 리포의 두 그림이 다른 이야기를 한다. 대지 이름이 같은 뿌리 둘(`app/cart/`·`features/cart/`)은 `assignUnits` 가 한 이름으로 합치므로, 그 뿌리 **밑에 있지 않은** 파일은 제 디렉터리로 접는다 |
| 엣지 | `import_edge` 를 폴더 쌍으로 집계. 쌍마다 **선 하나**, 종은 `static > http > dynamic > type` 중 가장 센 것 | 파일 다섯 쌍이 선 다섯이 되면 지도가 실타래다. `static` 과 `type` 이 함께 있는데 점선을 그리면 「타입만 가져온다」는 거짓말이 된다 |
| 밴드 | 접기는 **밴드가 정해진 뒤**에 한다. 폴더 층 = ① `folderBand` (§7.2 ① 을 폴더 경로에 적용하되 **마지막** `src/` 까지 벗긴다) ② 없으면 안에 든 파일이 **가장 많이** 앉은 층. 그 뒤 `band ≥ importer` 를 폴더 엣지로 한 번 더 돌린다 | 폴더 경로에 §7.2 ① 을 그냥 물으면 `src/features/cart/` 처럼 이름으로 모르는 폴더가 통째로 깊이 폴백으로 떨어진다 — 안의 `*.tsx` 는 패턴이 이미 「기능」이라 말했는데도. 최댓값도 최솟값도 파일 하나에 끌려간다(실측: 최댓값이면 여섯 노드 중 넷이 「공용 · 데이터」로 가라앉는다). 모노리포의 폴더는 `apps/desktop/src/components/` 라 앞을 안 벗기면 `components/**` 가 통째로 논다 |
| 순환·고립 | 접은 **뒤에** 폴더 단위로 다시 센다 | 한 폴더 안에서 닫힌 파일 순환은 「두 폴더가 서로를 가져다 쓴다」가 아니다. 반대로 폴더 안에서만 부르던 폴더는 접히면 자기 고리뿐이라 지도에 설 자리가 없다 |
| 상한 | 24 는 그대로. §7.4 의 ①②③ 은 폴더 노드를 건드리지 않는다 | 접기가 상한을 저절로 지킨다 |
| 렌더러 | **손대지 않는다** | `DependencyMap` 은 `bands`·`files`·`edges` 를 경로 문자열로 받는다 — 폴더 경로를 넣으면 그대로 그려진다. 05 §10 의 노드 수 = `frame_p95`(D81)도 이 접기로 피한다 |

**실측** (`fixtures/ipc/projectox-like` · Rust 덤프): **파일 69 → 노드 6 · 엣지 71 → 6**,
지도 밖 0. 가장 큰 폴더가 파일 30장을 한 노드로 접는다. `apps/desktop/src`(파일 106 ·
엣지 193)는 노드 5.

**지도가 안 서는 리포**: 대지가 `기타` 하나뿐이면 — `assignUnits` 의 네 규칙이 전부 물지
않았거나 2단계 디렉터리가 `MIN_FILES_FOR_UNIT` 에 못 미쳤다 — 접어도 한 덩어리다.
`repoMapStands()` 가 거짓이고 §8.5 두 종을 **내지 않는다**. 노드가 `MIN_REPO_NODES`(6)에
못 미쳐도 같다. 폴더 다섯 장짜리 그림에는 「이 프로젝트는 이런 구조구나」가 없다.

---

## 8. T2 문제 6종과 정답지

### 8.0 언제 어느 판을 굽는가 (D140)

`generateT2(req, kind)` 는 종 하나를 굽고, 종을 안 주면 §8.3·§8.4 의 순서(책임 배치 → 영향 반경 → 흐름 추적 → 의존성 방향)대로 만들 수 있는 첫 종을 낸다. **무엇을 언제 굽는지는 큐가 정한다** — 대지 하나에서 네 종을 다 굽되, 한 번에 한 장씩이다.

| | 규칙 | 왜 |
|---|---|---|
| 굽는 단위 | **세션당 한 장** | 책임 배치 한 장이 `t2.commit_files` 를 후보 커밋 수(≤ 60)만큼 부른다. 20대지를 한 번에 훑으면 1,200 쿼리다 |
| 순서 | **종이 바깥 고리** — `for kind of ORDER { for unit of units }` | 책임 배치만 실제 커밋을 정답지로 쓴다(정본 §2). 가장 강한 종을 리포 전체에 먼저 쓰고, 대지를 옮겨 가는 쪽이 사용자가 보는 변화도 크다 — 같은 대지의 네 종은 같은 지도 위 네 문제다 |
| 안 구운 조합 | `SELECT DISTINCT unit_id, kind FROM card WHERE track='t2'` 로 안다. 은퇴한 판도 센다 | 은퇴는 「그 조합을 다시 굽자」가 아니라 「그 판이 낡았다」는 뜻이다 |
| 한 세션 시도 상한 | 3 조합 | 지도가 안 서는 대지가 줄줄이 있어도 세션 열기가 멈추지 않는다 |
| 커밋 조회 | 종이 책임 배치가 **아니면** 건너뛴다 | 나머지 셋은 그래프만 본다(§8.3). 실측 IPC 69회 → 7회 |
| 다 구운 뒤 | 큐가 7일 창 밖으로 나온 옛 판을 다시 낸다 (02 §5.3) | 판 수는 `대지 수 × 4` + 리포 지도 두 종(§8.5 — 진입점 1 · 폴더 역할 최대 4)에서 멈춘다 |

**실측** (better-sqlite3 인프로세스 · 대지 20 · 커밋 1,200): 책임 배치 한 장 **4.2~8.4 ms · 쿼리 69회**, 그래프 3종 한 장 **0.5~6.5 ms · 쿼리 7회**. 69회 중 60이 `t2.commit_files` 다 — 실제 앱에서는 Tauri 왕복 60번이므로 여기가 유일한 값비싼 자리다.
...
유닛의 후보 커밋 **< 3** → 책임 배치 미생성, 홈에 「커밋이 적어 정답지가 그래프에서만 나옵니다」(정본이 든 project06 반례). 회전(§8.0)에서는 그 대지가 책임 배치 바퀴에서 조용히 빠지고 영향 반경 바퀴에서 다시 잡힌다 — 한 종도 못 내는 대지가 있고 그것이 정상이다(`two-commits`, D103). 「이것도 맞다고 생각해요」는 02 `appeal(track='t2', auto_verdict='wrong-pick', user_text=파일 경로, review_log_id, card_id)` 에 파일마다 1행 저장; 같은 `(card_id, user_text)` open 행이 3개면 그 문제의 sec 에 로컬 편입 제안(사용자 확인). 원격 집계는 없다(06) — 이슈 템플릿으로만. **왜**: 정답지가 커밋 1건이면 「정답 하나뿐인 정답지」다. 커밋은 한 사람이 그날 고른 경로일 뿐이므로 sec 와 피드백으로 넓힐 통로가 처음부터 있어야 한다.

### 8.5 리포 지도 두 종 — 진입점 · 폴더 역할 (D142)

§7.5 의 지도 위에 서는 두 종이다. 정답지가 **전부 기존 테이블**에서 나온다 —
`import_edge` 의 방향, `file.path` 의 이름, `unit` 의 뿌리. LLM 도 실행도 없다(정본 §5).

| 종 | 질문 | 정답지 | 함정 | 채점 |
|---|---|---|---|---|
| 진입점 `arch/entry` | `이 리포에서 밖에서 처음 들어오는 문은 어느 폴더인가요?` | core = **in-degree 0** 인 폴더 중 (out-degree > 0 ∨ 진입점 이름을 품음). sec = 진입점 이름은 품었는데 안에서도 부르는 폴더. trap = 나머지 전부, 사유는 in-degree | 가장 많이 쓰이는 폴더. 마지막 힌트가 그 이름을 댄다 | §8.2 와 동일 (`gradePicks`) |
| 폴더 역할 `arch/role` | `«lib/» 폴더는 왜 있나요?` 4지 | 밴드 라벨 넷(화면 / 기능 / 동작·통신 / 공용·데이터, §7.2)이 보기이고 정답은 그 색인. `payload.role = { folder, answer }` — **보기는 싣지 않는다**, `payload.bands` 가 곧 보기다 | 다른 밴드 라벨 셋 | `pct = 맞으면 100`, 진급 판정은 §8.2 (의존성 방향과 같은 자리) |

**진입점의 정답지가 초안과 다르다.** 초안은 「진입점 이름(`page.*`·`main.*`·`index.*`) ∪
(in-degree 0 ∧ out-degree > 0)」이었는데, **폴더 단위에서 그 합집합은 거의 모든 폴더다** —
TS 리포의 `index.ts` 는 문이 아니라 재수출 통이고 폴더마다 하나씩 있다. 그래서 이름은 독립
근거가 아니라 in-degree 0 위의 확인으로 쓰고, 이름은 있는데 안에서도 부르는 폴더는
`sec`(감점 없음)로 내린다. **「문처럼 생긴 것」과 「문」을 가르는 것이 이 문제가 가르치는
것이고**, 함정(많이 쓰이는 폴더)이 그 반대편이다.

내지 않는 조건: core 가 0개이거나, `MAX_ENTRY_CORE`(4)를 넘거나, **지도 노드의 절반을
넘거나**, 고를 수 있는 오답이 하나도 없을 때. 절반이 정답이면 아무 데나 찍어도 맞는다.
실측에서 `projectox-like` 는 in-degree 0 인 폴더가 넷이라 이 조건에 걸려 나오지 않는다 —
그 리포에 문이 넷이라는 뜻이고, 안 내는 것이 맞는 답이다.

**폴더 역할의 가장 큰 약점은 정답이 경로 패턴 휴리스틱(§7.2 ①)에 기댄다는 것이다.**
게이트가 둘이다.

1. 폴더 경로 자체가 §7.2 ① 에 걸려야 한다. **② 그래프 깊이로 추정된 층은 묻지 않는다** —
   추정이 틀린 채 「틀렸습니다」를 내면 한 번에 신뢰가 무너지고, 그 뒤로 사용자는 맞는
   판정도 믿지 않는다.
2. 그 패턴이 **지도가 실제로 앉힌 층과 같아야** 한다. 다르면 패턴이 거짓말한 것이다
   (아래층이 위층을 가져다 쓰는 리포에서 `relax` 가 폴더를 밀어 내린 경우).

여기에 `MIN_ROLE_MEMBERS`(2) — 파일 한 장짜리는 폴더가 아니다.

**물어볼 폴더는 지도에서 뺀다.** 밴드 행 라벨이 곧 4지의 보기라, 그 폴더가 지도에 그려져
있으면 정답을 읽어 주는 셈이다. 그래서 지도를 두 번 짓는다 — 첫 지도로 후보를 고르고, 그
폴더와 안의 파일을 뺀 두 번째 지도를 굽는다(§8.1 이 정답지를 지키려고 두 번 짓는 것과 같은
자리). 근거는 층 라벨이 아니라 **방향 집계**이고 힌트 ①이 그 문장이다 — 「N개 폴더가 이
폴더를 가져다 쓰고, 이 폴더는 M개를 가져다 씁니다」. 뺀 뒤 남은 지도가 `MIN_REPO_NODES` 의
절반에 못 미치면 내지 않는다: 그림 없이 숫자만 남으면 「지도를 읽어라」가 아니라 「숫자를
외워라」가 된다.

**회전** (§8.0): 두 종은 지도가 대지마다 다르지 않다. `t2Todo` 는 대지를 **회전 커서**로만
쓰고 — `SELECT DISTINCT unit_id, kind` 로 「같은 종을 두 번 굽지 않는다」를 세려면 서로 다른
대지에 매달 수밖에 없다 — 몇 번째 후보 폴더를 물을지는 `targetIndex` 가 든다. 진입점은
리포에 하나뿐인 물음이라 한 장, 폴더 역할은 넷에서 멈춘다(§7.2 ① 이 이름으로 아는 디렉터리는
열둘이고, 한 리포에서 다섯 번 넘게 물으면 같은 교훈의 반복이다). 종을 **안 주면** 두 종은
시도하지 않는다: 입력이 대지 파일일 때 굽으면 「대지 하나짜리 리포 지도」라는 거짓 그림이 된다.
...
| 「이 폴더는 왜 있나」(§8.5)의 정답이 경로 패턴 휴리스틱(§7.2 ①)이라 틀린 채 「틀렸습니다」를 낼 수 있다 | 패턴이 **이름으로 아는** 폴더에만 낸다. 깊이로 추정된 층(§7.2 ②)은 묻지 않고, 패턴과 지도가 앉힌 층이 갈리면 그 폴더도 뺀다. 한 번 틀린 판정을 내면 그 뒤로 맞는 판정도 안 믿는다 |

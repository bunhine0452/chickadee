...
한 문제 지도 **≤ 24 노드**. 범위 = 유닛(기능 폴더) + 1-hop 이웃. 초과 시 ① 유닛 밖 노드를 디렉터리로 접어 폴더 노드 `lib/ (3)` — **정답지(core/sec)에 든 파일은 절대 접지 않음** ② 그래도 초과면 정답지 밖 · in-degree 1 인 밴드 3 잎을 「지도 밖」 각주로. 접힌 노드를 고르면 그 안 파일이 core 에 없으므로 wrong 처리하되 사유 「접힌 폴더 — 안쪽 파일을 묻는 문제가 아님」.
...
trap = 지도 안 ∖ (core ∪ sec) 중 core 의 1-hop 이웃 → 관계별 사유 템플릿
```

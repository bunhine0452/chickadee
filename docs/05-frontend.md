...
포커스 규칙:
- 교정지 마운트 → `article.ps` 로 포커스(스크롤 0). 채점 → 포커스는 그대로, `FeedbackSlot` 이 `aria-live` 로 읽힌다. 사다리 열림 → 현재 단 `tab` 으로. 아래층 점프 → 새 교정지. 복귀 → `LinkPara`. 요약 → 「홈으로」 버튼. 세션 닫힘 → `returnFocusId`.
...
| SVG `<use>` Dee | 화면당 ≤ 40 인스턴스. 홈 시트 12장 초과 시 시트 목록 **윈도잉**(`content-visibility:auto` + `contain-intrinsic-size` 를 먼저, 부족하면 가상 스크롤) | `<use>` 는 인스턴스마다 6 경로(수천 점) 재래스터, 캐시 없음 |

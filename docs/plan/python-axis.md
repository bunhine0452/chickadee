...
> **2026-09-05 (S6) — 같은 여덟을 tree-sitter 쿼리로 다시 쟀다.** 위 표는 `ast` 노드 수이고
> 아래는 **사전 쿼리가 실제로 잡는 수**다(`adelie` + `ECC` 두 리포, `MonggleMonggle` 제외):
> `assignment` 6,020곳/186파일 · `if-statement` 2,115/120 · `function-definition` 2,154/181 ·
> `comparison` 1,804/155 · `arithmetic` **265/69** · `return-statement` 1,525/150 ·
> `boolean-literal` 1,199/151 · `while-loop` 19/14.
> **`arithmetic` 만 자릿수가 다르다** — S8 이 피연산자 종류를 열거해 좁힌 결과이고(§1.7 수리),
> 나머지 일곱은 `ast` 계수와 같은 자릿수다. 쿼리와 `ast` 가 같은 것을 세고 있다는 뜻이다.
...
> **2026-09-05 갱신 (S6) — 이 절 앞에 0부가 붙었고, 사전 24장이 실물로 섰다.**
> 아래 §2.1·§2.2 는 **0부가 생기기 전의 설계표**다. 갱신된 부 배치는
> [`docs/curriculum/py.md`](../curriculum/py.md) §1.5.4 가 정본이고, 이 절은 그 뺄셈의 원본으로 남긴다.
>
> | 무엇 | 이 계획을 쓸 때 | 오늘 |
> |---|---|---|
> | 사전 장 수 | 8 | **24** (0부 18 + 1부 6) |
> | `.scm` 쿼리 | 개념 8 + 시스템 2 | 개념 24 + 시스템 2 |
> | 골든 | **없음** | `fixtures/golden/py/**` — 개념 24 × (양성 3 · 음성 2) + 함정 3 |
> | `essential` | 8 | **24**, 전부 깊이 ≤ 2 → 0장 후보 24/24 (하루 두 장이면 12일) |
> | 사용처 0곳인 개념 | 안 쟀다 | **하나** — `py/implicit-conversion` (§2.5) |
>
> **§1.2 의 「바닥 8 → +중심 16」 예측이 반쯤 맞았다.** 그 표는 8 → 24 장이 `adelie` 의 줄 덮음을
> 37.6 % → 75.9 % 로 올린다고 적었는데, 실제로 선 24장은 그 「중심 16」이 아니라 **0부 16장**이다.
> 겹치는 것은 여섯(`string-literal`·`f-string`·`truthiness`·`is-identity`·`list-literal` ·
> 그리고 세는 방식이 다른 `number-literal`)이고, 나머지 열(`attribute-access` 14,495곳 ·
> `call-expression` 14,074 · `import` 2,185 · `index-access` 1,718 · `dict-literal` 1,394 ·
> `none-value` 1,180 · `for-in` 605 · `list-append` 542 · `list-comprehension` 181 ·
> `tuple-unpacking` 75)은 **아직 없다.** 줄 덮음은 다시 안 쟀다 — 이 세션은 캡처 수만 쟀고,
> 줄 단위 덮음을 재려면 §1.1 의 스크립트를 다시 돌려야 한다.
...
### 2.2-a 오늘 실제로 선 24장 — 캡처 실측 (2026-09-05 · S6)

`adelie`(py 139파일) + `ECC`(py 63) 두 리포를 사전의 `.scm` 으로 직접 돌린 수다.
`MonggleMonggle` 은 이 세션에서 안 쟀다. 순서는 `dictionary/py/_lang.yaml` 의 `essential` 그대로다.

| # | id | 부 | adelie 곳·파일 | ECC 곳·파일 | 합 |
|---|---|---|---|---|---|
| 1 | `py/assignment` | 0부 G | 4,471 · 131 | 1,549 · 55 | 6,020 · 186 |
| 2 | `py/number-literal` | 0부 A | 531 · 93 | 214 · 28 | 745 · 121 |
| 3 | `py/value-bits` | 0부 A | 3 · 2 | 4 · 1 | **7 · 3** |
| 4 | `py/integer-limit` | 0부 A | 2 · 1 | 1 · 1 | **3 · 2** |
| 5 | `py/string-literal` | 0부 C | 11,679 · 131 | 4,110 · 57 | 15,789 · 188 |
| 6 | `py/text-length` | 0부 C | 311 · 82 | **49 · 9** | 360 · 91 |
| 7 | `py/arithmetic` | 0부 E | 193 · 56 | **72 · 13** | 265 · 69 |
| 8 | `py/f-string` | 0부 C | 1,664 · 80 | 557 · 27 | 2,221 · 107 |
| 9 | `py/integer-division` | 0부 B | 10 · 4 | 5 · 2 | **15 · 6** |
| 10 | `py/float-inexact` | 0부 B | 19 · 9 | 12 · 1 | **31 · 10** |
| 11 | `py/boolean-literal` | 0부 D | 893 · 109 | 306 · 42 | 1,199 · 151 |
| 12 | `py/operator-precedence` | 0부 E | 15 · 3 | 9 · 4 | **24 · 7** |
| 13 | `py/explicit-conversion` | 0부 F | 213 · 59 | **72 · 21** | 285 · 80 |
| 14 | `py/truthiness` | 0부 D | 524 · 70 | 164 · 23 | 688 · 93 |
| 15 | `py/bool-op-value` | 0부 E | 59 · 29 | 23 · 12 | **82 · 41** |
| 16 | `py/implicit-conversion` | 0부 F | **0 · 0** | **0 · 0** | **0 · 0** |
| 17 | `py/comparison` | 0부 H | 1,321 · 115 | 483 · 40 | 1,804 · 155 |
| 18 | `py/is-identity` | 0부 H | 283 · 63 | **79 · 27** | 362 · 90 |
| 19 | `py/if-statement` | 1부 | 1,619 · 83 | 496 · 37 | 2,115 · 120 |
| 20 | `py/list-literal` | 1부 | 1,129 · 121 | 458 · 48 | 1,587 · 169 |
| 21 | `py/reference-binding` | 1부 | 142 · 52 | 126 · 19 | 268 · 71 |
| 22 | `py/while-loop` | 1부 | 15 · 12 | 4 · 2 | **19 · 14** |
| 23 | `py/function-definition` | 1부 | 1,630 · 129 | 524 · 52 | 2,154 · 181 |
| 24 | `py/return-statement` | 1부 | 1,190 · 109 | 335 · 41 | 1,525 · 150 |

**굵은 것이 얇은 판 열둘이다** — 두 리포 중 한쪽에서라도 100곳 미만. `_lang.yaml` 이 그 열둘을
두꺼운 판 사이에 **둘씩** 흩어 놓는다(셋이 붙는 자리가 없다). 근거는 `ts/_lang.yaml` 이 세운 것과
같다: 「네 코드엔 없다」만 사흘 내리 나오면 D177 규칙 ①이 그동안 한 번도 안 걸린다.

**대조 — 문서의 옛 실측과 어긋난 자리 셋.** ① `is-identity` 283/79 는 §1.4 의 `ast` 계수와
**한 곳도 안 틀린다.** `boolean-literal`·`comparison`·`function-definition`·`return-statement`·
`while-loop` 도 같다. ② `f-string` 2,221 은 §1.4 의 1,476(adelie+ECC)보다 크다 — 이 쿼리는
f-문자열 하나가 아니라 **중괄호 한 칸**을 사용처로 세기 때문이다. ③ `string-literal` 15,789 는
§1.4 의 20,596 보다 작다 — f-문자열·바이트 문자열이 빠졌다(`py/f-string` 이 가져갔다).

...
| 코드 주석 · 식별자 | 영어 |
| 사용자에게 보이는 앱 문구(UI·오류 문구·사전 YAML) | **한국어가 정본, 영어 병기**(D117·D118) — 새 문구는 `packages/i18n` 의 `t()` 를 거치고 `ko` 를 먼저 쓴다 |
...
한국어가 맞다. 앱 문구는 조판 규칙(`keep-all`·`--measure` 36em·조사 헬퍼)이 한국어 전제라 `ko` 가 정본이고,
`en` 은 05 §9 의 로케일 축(행 길이 45~68자)을 따로 따른다.
...
- **얇은 Rust.** 줄 예산은 **폐지됐다**(D181 — 줄 수는 보고만 한다). 지키는 것은 도메인 어휘
